...
def show_add_task_to_projects(wf, api_key, task, due=None):
	...
	for project in result:
		params = dict(token=api_key, content=task, priority=1, project_id=project['id'])
		if due:
...
def main(wf):
	...
	if query == 't' or query == 'q' or query == 'c':
		...
		if nterms > 0:
			...
			queries_str = '["today"]'
		params = dict(token=api_key.encode('utf-8'), queries=queries_str, js_date='1')
		r = web.get(url, params)
		r.raise_for_status()
		result = r.json()
		for each in result:
			for item in each['data']:
				wf.add_item(title=item['content'],
										subtitle='Due on {}, click to complete'.format(item['due_date']),
